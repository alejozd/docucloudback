...
class FSEMonitorJob {
  ...
  async checkCriticalSupplies() {
    try {
      ...
      const critical = fbos.filter(
        (f) => f.daysOfSupplies !== null && f.daysOfSupplies < 30,
      );
      ...
      DebugLogger.log(
        "JOBS",
        `Sent critical alert for ${critical.length} FBOs`,
      );
    } catch (error) {
  ...
  async sendDailySummary() {
    try {
      ...
      const avgDays =
        validDays.length > 0
          ? validDays.reduce((sum, f) => sum + f.daysOfSupplies, 0) /
            validDays.length
          : null;
      ...
      const attention = fbos.filter(
        (f) =>
          f.daysOfSupplies !== null &&
          f.daysOfSupplies < 60 &&
          f.daysOfSupplies >= 30,
      );
